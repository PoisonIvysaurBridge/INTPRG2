...
public class Date{
    ...
    // other methods
    public boolean isBefore(Date given)
    {
        boolean before = false;
        if (this.nYr <= given.nYr && this.nMonth <= given.nMonth && this.nDay < given.nDay)
        {
        	before = true;
        }
    ...
    public void display(){
        if (nDay == 0 || nMonth == 0 || nYr == 0)
            System.out.println("Incomplete data");
        System.out.println("Date is (mm/dd/yyyy): "+nMonth+"/"+nDay+"/"+nYr);
    }
}